...
async def cmd_start(message: Message):
    text = (
        ...
        "/edit_player - Редактировать данные профиля игрока\n"
        "/del_player - Удалить игрока\n"
        "/record - Добавить ручной рекорд (прохождение или прогресс)\n"
        "/del_record - Удалить рекорд\n"
        "/info_update - Запустить синхронизацию с сайтом Demonlist\n"
        "/backup - Скачать резервную копию базы данных\n"
        "/restore - Восстановить базу данных (ответом на файл database.db)\n"
        "/toggle_notifications - Включить/выключить уведомления в канале\n"
        "/update - Скачать обновления с GitHub и перезапустить бота\n"
        "/restart - Быстрый перезапуск бота\n\n"
        "👑 Команды владельца (только для ROOT_ID):\n"
        "/add_admin [TG_ID] - Выдать права администратора\n"
        "/del_admin [TG_ID] - Забрать права администратора"
    )
